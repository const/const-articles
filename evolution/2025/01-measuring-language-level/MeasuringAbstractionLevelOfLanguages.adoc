...
In C, reasoning about a tagged union scales like `(amount of referrers) * (amount of referred cases)`, because every extraction site must account for all possible variants. In Java, reasoning cost scales like `( (amount of referrers) + (amount of referred cases) ) * (used contract size)`, because the reasoning graph is partitioned: callers reason only up to the interface boundary, and implementers reason from it. Since reasoning cost dominates overall cognitive load, the difference between these approaches stems primarily from reasoning topology rather than syntax or learning overhead.

This structural advantage holds for modifications as well. Adding a new variant in C may require visiting every referrer site. In Java, it only requires adding a new class that implements the interface. The caveat is that if the interface violates the Single Responsibility Principle, the `(used contract size)` factor grows, which quickly erodes the partitioning benefit.

NOTE: Functional Programming makes single-method objects exceptionally convenient. A single-method object is the second-simplest contract possible, and much of FP's practical power derives from the fact that when cognitive cost scales like `( (amount of referrers) + (amount of referred cases) ) * (used contract size)`, the `(used contract size)` component is kept minimal.
...
Some modern languages deliberately blend levels for pragmatic reasons. Go restricts Level 4 features (restricted inheritance, limited generics initially) to keep it readable, accepting higher boilerplate for predictable scaling. However, Go has enough level 4 features still to be classified as a level 4 language.
...
Thus, I think dynamically-typed level 5 languages is not feasible. The static typing will be likely added later anyway if language is used to develop large code bases, so doing this intermediate step will cause spending efforts later to bolt on static typing while solving conflicts and carrying some dead weight of dynamically-typed legacy.
