...
This article presents a semiformal, ordinal scale for classifying the abstraction level of programming languages based on their predominant connection types along the growth path. The scale is not a continuous numeric measurement but a structured ranking (Levels 1–4) that relies on expert evaluation of language idioms and scaling mechanisms.

The model presented here could help with the following:

* It outlines a framework for evaluating existing technologies and better predicting their impact on the software development process.
...
This is the first article in the series, and its primary goal is to introduce the idea and outline its applicability for analysis of programming languages and other domains. The primary goal is to provide a lens for discussion rather than a formal executable methodology. The followup articles will explore this idea further.
...
| 0.1.6   | 2026-07-15 | Added more references, small fixes
| 0.1.5   | 2026-05-18 | Added Coq LLM discussion, appendix on memory
| 0.1.4   | 2026-05-06 | Added Coq discussion, minor clarifications on level translation, style fixes
| 0.1.3   | 2026-05-02 | Clarified reasoning cost topology, integrated practical diagnostics, refined Level 5 framing
| 0.1.2   | 2026-04-30 | Correcting incorrect statements that caused overclaims
| 0.1.1   | 2025-10-12 a|
...
2. *Structured Programming*: C, Pascal, FORTRAN 77 (however, the transition to structured programming completed only at FORTRAN 90). These languages implement structured programming approach and use blocks, procedures, data structures, hierarchies, and recursion to organize the code.
...
If we try to introduce a better scale that evaluates language level, we would need more formal criteria that sort languages approximately in this order. A naive approach would be using "likeness" criteria, classifying languages based on paradigm and assigning numbers depending on the paradigm.

However, this approach does not help with non-mainstream paradigms like Logic Programming, and within the Logic Programming paradigm, there could be languages with apparently different abstraction levels (for example, Verse vs. Prolog). Thus, this approach could not be used to classify languages reliably as it is too subjective. To classify languages more formally, we need formal (or at least semiformal) criteria rather than intuitive ones.
...
Software maintenance is fundamentally a cognitive task. To understand why abstraction levels matter, we must examine it from two complementary perspectives: the **per-task experience** and the **project history**.

=== The Per-Task View: How We Encounter a Change

When modifying behavior, developers follow four cognitive steps:

1. Locate *all* relevant places that affect the target behavior.
2. Understand what changes will produce the desired outcome without introducing side effects.
3. Implement these changes.
4. Test the result.

The cost of steps 1 and 2 dominates development time. For any given task, the codebase divides into four quadrants based on whether code *should* have been examined and whether it *actually was*:

[cols="2,3,3",options="header"]
|===
| | **Not Viewed** | **Viewed**
| **Should Have Been Viewed**
| *Missed Context* → Bugs, regressions, incomplete fixes
| *Productive Work* → Necessary reasoning, bounded by human working memory
| **Should Not Have Been Viewed**
| *Ideal Code* → Zero cognitive load for this task; the goal of abstraction
| *Cognitive Pollution* → Context switching, wasted time, reduced quality
|===

The critical question becomes: _what determines an abstraction’s power to partition reasoning space so that the *Ideal Code* quadrant is maximized?_ Empirically, higher-level languages make complex systems easier to maintain, suggesting that abstraction level directly correlates with how effectively a language’s constructs divide the reasoning graph into safely ignorable boundaries.

=== The Project History View: How Complexity Accumulates
Over time, behavioral complexity grows due to changing requirements, domain understanding, bug fixes, and external integrations. The cognitive cost of the per-task steps does not remain constant. In different languages, the cognitive complexity growth curve follows different trajectories:

image::images/diagram1.png[complexity growth diagram]
.The diagram shows cognitive complexity (y‑axis) vs. behavioral complexity (x‑axis). Lower‑level languages start with lower initial complexity but experience steeper growth. Higher‑level languages have higher entry cost but flatter growth.

Key observations reflected in this curve:

* Lower-level languages have lower entry costs. Their building blocks are easier to grasp initially, but it is harder to maintain connections between them in the mind.
* Behavioral complexity causes non-linear growth in cognitive complexity. Eventually, every program reaches an unmaintainable threshold where the per-task cost exceeds human capacity.
* Higher-level languages slow this growth. They enable us to implement more complex behavior before hitting that threshold.

The "program" here is not limited to a single OS application. A set of related microservices maintained by a team, sharing concepts and code, constitutes a single reasoning scope.

[NOTE]
====
**Observations in favor of the Flat → Structured "flatter curve"**

While large-scale maintenance studies are rare, cognitive psychology research directly supports the scaling benefit of structured control flow.

* A controlled experiment link:https://link.springer.com/article/10.3758/BF03203778[Sheppard & Curtis, 1981] asked programmers to anticipate the next line of code in structured vs. unstructured programs. Structured code was significantly more comprehensible — and critically, *the advantage was more pronounced for long, complex programs*. The benefit is negligible at ~50 LOC (where the entire program fits in working memory) but becomes essential at scale.
* Dijkstra's seminal letter link:https://www.cs.utexas.edu/~EWD/transcriptions/EWD02xx/EWD215.html["Go To Statement Considered Harmful" (EWD215)] provides the foundational explanation: unstructured jumps force the reader to maintain a global timeline of possible execution paths, while structured blocks enable local reasoning and chunking.

**Observations in favor of the Structured → OOP/FP "flatter curve"**

Direct, freely accessible studies comparing procedural vs. contract-based maintenance scaling are rare. However, proxy evidence supports the "flatter curve" argument:

* An arXiv study of ~785K Java methods link:https://arxiv.org/pdf/2205.01842[Chowdhury & Holmes, 2022] found that keeping methods ≤24 SLOC reduces change- and bug-proneness. Decomposing large methods into smaller, contract-bounded units lowers maintenance effort.
* Recent cognitive-load research link:https://arxiv.org/pdf/2103.00821[Antinyan et al., 2021] suggests that structural representations may reduce comprehension effort.
* Observation: Large C codebases (Linux kernel, PostgreSQL) frequently implement abstraction patterns typically associated with higher‑level languages: function pointers for polymorphic dispatch, opaque structs for data hiding, manual vtables for interface‑like behavior. This suggests that when behavioral complexity grows, developers naturally reach for stronger abstraction, even if the language doesn't provide them natively.
====

=== Connecting the Views

Abstraction levels bend the project-history curve by partitioning the per-task reasoning graph. Each new level introduces boundaries that push more code into the *Ideal Code* quadrant for every new change request. A reliable measuring approach of programming language levels allows us to predict which technologies will sustain this partitioning as systems grow, and which will force developers into cognitive overload. For example, see https://news.ycombinator.com/item?id=18442941[the post about Oracle Database development process] that describes the realities of working with a large codebase.

So, a reliable measuring approach will allow us to make the following deductions:
...
* It would allow us to explore if some technology is a breakthrough in complexity management area, or it is just the same thing again, and only minor improvements could be expected. There are a lot of technologies that are claiming to be a new generation, but after hype calms down, there is not so much difference from existing ones.
...
[NOTE]
====
**Clarifying Level 1 vs Level 2**:

- **Level 1** is execution based *only on the current state*: it retains no memory of past steps and has no notion of a “program flow” beyond what is encoded in registers and memory.
** A human operating a calculator: each key press transforms the display based solely on the current value and the operation; the calculator does not “know” the sequence of previous keys.
** A CPU logically (according to its contract) does the same: it fetches an instruction, reads registers and memory, updates them, and then forgets everything except the new state. The CPU has no concept of “program” – only state transformation.

**Level 2** is defined by the sequential machine contract (the ISA), which guarantees program‑order semantics across discrete state‑transforming operations. It provides the illusion of logical sequential execution by hiding microarchitectural complexity—speculation, out‑of‑order execution, and pipelining—behind a strict observable contract. The sequential order is emergent: the CPU reorders and optimizes internally, but the ISA ensures that *observable* state evolves strictly in program order.

We do not model hardware microarchitecture because silicon already interprets Level 2; Level 1 execution is handled transparently by the hardware’s state‑update machinery.

**Compilation targets Level 2** because it is the lowest layer where sequential reasoning is preserved and portable across microarchitectures. Higher‑level abstractions (structs, procedures, interfaces, polymorphism) are erased to Level 2 because machines do not need reasoning boundaries: they need a predictable, sequential substrate. The hardware then further erases *instruction order* at the physical level, but the ISA contract guarantees that the programmer sees sequential behavior.

This boundary explains why optimization often means erasing higher‑level reasoning: we trade human‑comprehensible structure for machine‑executable efficiency.
====
...
Structured links add a new element to name interpretation: a context. One of the simplest things is a local variable. To understand what it could contain, we need to understand the current stack frame. In the case of recursive calls, the variable value could be different for different stack frames. Let's consider a simple recursive factorial function in C:
...
Also, conditional statements use relative links to their components. The conditional statement does not refer to statements by global names like labels or line numbers. The statements are organized in blocks, and `if` composes these blocks into a new whole. There could be nested `if` statements. The same is for expressions, which form a tree.
...
This is because Java's version does not care about what the type exactly is. It only cares about its implementation conforming to the contract. So, knowing less frees our reasoning about the program from unneeded details, and it also makes our code more widely applicable.

The difference from level 3 reasoning is that the reasoning is split into two parts, up to some formal criteria for the referrer, and from formal criteria for the referred entity.
...
When reasoning about a value that can be one of several variants, cognitive cost scales differently depending on the abstraction:

* **Sum type / tagged union (Level 3)**:
`(number of referrers) × (number of variants)`
Every extraction site must account for all possible variants.

* **Interface / contract (Level 4)**:
`({zwsp}(number of referrers) + (number of variants){zwsp}) × (used contract size)`
Callers reason only up to the interface boundary; implementers reason from it.

In many contexts, reasoning topology is a dominant factor in cognitive load differences between these approaches, though syntax, learning overhead, and tooling also contribute.

[NOTE]
====
Functional Programming makes single-method objects exceptionally convenient. A single-method object is the second-simplest contract possible, and much of FP's practical power derives from the fact that when cognitive cost scales like `({zwsp}(referrers) + (variants){zwsp}) × (contract_size)`, the `(contract_size)` component is kept minimal.
====
...
To sum it up, at level 4, the reasoning cost has reduced further. At level 3, we are able to reason by induction on the source tree. At level 4, we do not need to reason on the entire tree, we can split the reasoning process up to some checkpoints, where some formal criteria are specified. The key to the reasoning process is in selecting such suitable formal criteria (interfaces, generics, and so on). Choosing the wrong criteria might increase the cost of reasoning instead.
...
This principle states that class should be responsible for one thing. If there are multiple things the class is responsible for, then when reasoning about the class behavior we need to consider interactions of these responsibilities. For example, when changing some behavior, we need to consider all responsibilities of the class. Thus, we have multiplicative cost of change in the worst case.

**Framework interpretation**: Isolating responsibilities partitions the reasoning graph and reduce contract size involved into individual reasoning steps.
...
**Framework interpretation**: Boundaries stop reasoning traversal. Extension happens by adding new implementations to the existing contract, preserving the caller's reasoning scope.
...
**Framework interpretation**: Contract adherence guarantees that substituting implementations does not expand the caller's reasoning scope. Violations force path tracing through concrete types.
...
**Framework interpretation**: Minimizing contract size directly minimizes the reasoning tax. Unused methods force the client to view and mentally filter them, polluting the cognitive context.
...
**Framework interpretation**: Abstract contracts decouple reasoning from implementation details. The caller reasons up to the interface; the implementer reasons from it. This partitioning is the core mechanism that reduces cognitive scaling cost.

===== Cost of SOLID

The thing that is missed in the SOLID is that contract itself has the cost: it needs to be formulated, maintained, and distributed. Also, components could not rely on things that are not in the contract. For widely distributed library, the design should be very careful with respect to SOLID, but for private microservice code we could cut corners, to make development cheaper and refactor it later when needed. The total cost of contract depends on uses and implementations, if there is a single supplier of the interface and a single producer, the benefit of a separate interface must be weighed against its maintenance overhead.
...
The community first addressed the problem by adopting 'anemic' POJOs as a pattern that was later supported by tools like Lombok. POJOs were often critiqued as non-true OOP from theoretical point of view. The later records and sealed types were added to Java, so level 3 types were directly supported with simplified and lower-cost level 3 reasoning where it makes sense.
...
Later, he specifically addresses high-level languages. The part is very important, and I really do not know what to drop from it without losing some important parts.
...
This is the point where I disagree with F. Brooks. The higher level languages affect cognitive scaling, it should be not be judged basing only on accidental complexity reduction. F. Brooks focuses on complexity snapshot rather than on complexity dynamics. The problems in higher-level languages have lower growth of the cognitive complexity growth with the growth of the behavior complexity. This saving does not come from what we are thinking about, but mainly from what we do not need to think about and still make correct decisions about the program.

For example, for a well-designed interface, we could stop thinking in that direction after examining the interface. We do not need to think about every implementation of this. Also, when implementing an interface, we do not need to think about every place that uses it; the interface is a contract that specifies what we need to do to support all these places.

Higher-level abstractions do reduce code duplication through DRY: generics, interfaces, dependency injection, higher-order functions. And this reduction is often noticeable. However, the reduction is rarely as large as intuitively expected, and abstraction layers sometimes add boilerplate or setup overhead.

However, the hidden cost of DRY is that reducing duplication couples previously independent sites. For some tasks, such as adapting a shared contract, the number of affected locations increases, expanding the reasoning scope and introducing unexpected links. The primary cognitive benefit of abstraction is not simply "less code"; it is scoped reasoning. By partitioning the codebase, abstractions ensure that for most tasks, you only need to examine a bounded subset. Well-designed abstractions maximize the code you safely ignore while keeping coupling explicit and localized.
...
**Brooks was right, but incomplete**. There is no silver bullet: no single development promises an order-of-magnitude improvement in productivity *for all tasks*. However, higher-level languages provide **heavier ammunition**, which is more difficult to handle and requires more sophisticated tooling, but can solve problems that lower-level languages cannot sustain.

The primary benefit of high-level languages is not immediate cost saving, but the ability to implement applications with more complex behavior *before* cognitive load becomes unmanageable. Lower-level languages fail earlier; higher-level languages extend the horizon.

[NOTE]
====
**Optimization as Level Descent**: Performance tuning is fundamentally a process of erasing higher-level reasoning boundaries to align with lower-level execution semantics. This is not inherently negative: erasure is how we pay for speed. However, the erasure increases reasoning complexity, and the quote below is a reflection on this.
[quote, Donald Knuth]
...
If we consider cognitive scalability, we need to understand which concepts are on the *growth path* when we try to increase behavior complexity. By ‘growth path’ we mean the set of language concepts that are added or scaled as program size increases – the abstractions that dominate the program’s structure in large, idiomatic code. Concepts on growth path are linked to each other and should grow when behavioral complexity grows:
...
| Java, C#, {cpp} | 4 - Formal | OOFP | Interfaces, classes, methods, generics
| Haskell, OCaml | 4 - Formal | FOOP | Functions, type classes, generics, data types
|===

[NOTE]
====
**Functional Programming as a bridge**: Pure FP can be viewed as a transitional stage between structured programming and OOP/FP hybrids. It excels at formalizing Level 3 constructs (algebraic data types, pattern matching) and introduces Level 4 reasoning via higher-order functions and generics. However, its treatment of complex existential types (interfaces) is often less ergonomic than OOP's, which is why mainstream languages tend toward hybrid models.
====
...
This example shows, that we need to consider links on the growth path, rather than all links of the concept. So we need to classify the level of the sea, rather than the level of the islands.
...
Some modern languages deliberately blend levels for pragmatic reasons. Go restricts classical inheritance in favor of interface composition and struct embedding, keeping the growth path focused on Level 4 contracts rather than hierarchy traversal. Generics were added later (1.18) to support universal quantification without compromising readability.

NOTE: A language's abstraction level defines the *ceiling* of cognitive scalability for idiomatic usage, not a mandatory coding style. You can write level 3 procedural code in a level 4 language like Java, but attempting to reason beyond that ceiling requires emulating higher‑level constructs through frameworks or conventions, which merely shifts the cognitive mapping cost back to the developer. This means: choose a language whose ceiling comfortably exceeds your anticipated complexity, then progressively adopt its native higher‑level constructs *as the growth path demands*.

=== Level 0. Human-Executed Programs
Before any external automation, computation is defined by an algorithm: a sequence of instructions designed for a human to execute. The term "algorithm" itself originates from the work of Arabic mathematicians to describe such human-executed procedures, much like a culinary recipe describes a process for a chef. In this stage, there is no separation between algorithm and executor; the human mind holds both the state and performs the operations.

The modern analogue is mentally calculating a complex formula or tracing a small algorithm on paper without executing it. Level 0 establishes the concept of the stored algorithm: the separation of "instructions" from "data" exists, but the execution engine is biological, not mechanical.

=== Level 1. Calculator Paradigm
The transition from Level 0 to Level 1 occurs when we invent a device that can execute specific operations of the algorithm.

In Level 0, the human acts as the CPU, ALU, and Control Unit. In Level 1, the human remains the Control Unit: sequencing steps and managing data, but the ALU is offloaded to a mechanical or electronic device. The "program" is still largely in the human's mind, but the steps are no longer mental calculations; they are physical interactions with a state-transforming machine.

The language of the simple non-programmable calculator conforms to this paradigm: each key press modifies the state, but the user still works with a single value on the screen. I include this paradigm mostly for completeness.
...
The transition from level 2 to level 3 is somewhat vague, and ALGOL 58 and ALGOL 60 were the first languages in that generation. However, Pascal and C are probably more widely known, and C is still a very popular language for system programming. The reasoning behind transition is documented in Edsger Dijkstra's letter "Go To Statement Considered Harmful", published in March 1968. A good analysis of the article from modern perspective is done by David R. Tribble in the article http://david.tribble.com/text/goto.html[Goto Statement Considered Harmful: A Retrospective].
...
While FP and OOP belong to level 4, there were a number of code generator, macros, and design patterns that supported it in level 3 languages.

For example, the most of C-based UI toolkits implement something poor-man lambdas using function pointer with void pointer to implement handling of events for windows. https://en.wikipedia.org/wiki/IBM_System_Object_Model[IBM SOM] and https://en.wikipedia.org/wiki/Common_Object_Request_Broker_Architecture[CORBA] code generators generated C code that implements and invokes interfaces. {cpp} began as 'C with Classes' (1979), a general-purpose language extension to C.

Its first implementation, Cfront, was a source-to-source compiler that translated {cpp} code to C for portability and bootstrapping (a technique later common in DSL toolchains, though C++ itself was never domain-specific). Unix file handles and everything-is-a-file are actually example of interface concept implemented as a design pattern. For more examples, see https://www.cs.rit.edu/~ats/books/ooc.pdf[Object-Oriented Programming With ANSI-C] by Axel-Tobias Schreiner.
...
=== Compilation as Controlled Level Descent

Higher abstraction level is *erased* during compilation to lower levels to enable execution:

* Level 4 objects, interfaces, and dependency contracts → Level 3 IR (SSA, control flow graphs)
* Level 3 structured composition → Level 2 machine code (flat addresses, explicit registers, sequential control flow)
* Level 2 machine code → Level 1 execution (parallel, state-driven, order-erased by hardware)

Compilation stops at Level 2 because processors are interpreters of the ISA contract. Translating further to Level 1 would mean executing instructions one by one in the mind – which is precisely what we offloaded to machines in the first place.

*Erasure is not loss, it is the mechanism by which we ground our description of computation*. The source code contains guidelines necessary for human reasoning (local scope, compositional contracts, polymorphic dispatch), the compilation process uses these constructs as scaffolding to create executable code.

This explains why "optimization" often feels like losing abstraction: inlining erases Level 4 composition into Level 3 control flow; monomorphization erases Level 4 polymorphism into Level 3 concrete dispatch; loop unrolling erases Level 3 iteration into Level 2 parallel instructions. The tension between human reasoning and machine execution is structural, not accidental.
...
This allows us to restrict reasoning scope, when we are locating pieces that affect specific behavior, or trying to predict effect of the change. So each new abstraction level on primary evolution vector greatly reduces software development and maintenance costs for big idiomatic programs.
...
* Concepts of the previous levels are often restricted, so there are additional conditions on when they could appear. For example, `GO TO` variants like `break` or `continue` should follow some block structure. Or typed references could not point to arbitrary object in statically-typed languages.

* The concept of the next level are actually used in the languages of the previous level, but they are not natively supported. And developers use design patterns, whitespaces, comments, code generators, macros, or other means to support them.

* These concepts start to be used, because they really help to fight complexity. So, more the complex domain is, the more likely that concepts will appear. And using these concepts on the previous level is considered as example of a good design.
...
This section demonstrates how the framework can be extrapolated to predict future abstraction levels. It outlines a working hypothesis for Level 5 (Holon/System-Oriented Programming) and is not yet validated by mature language implementations. Concrete syntax and compiler architecture are being developed as research prototypes.
...
Initially, when I started this analysis, I hoped that Language-Oriented Programming is the next big thing. So, the creativity of the software developers will be unleashed in the area of designing the languages. However, LOP does not directly match criteria above. After further research, I have narrowed down possible candidates to a single candidate that match all these criteria, and this candidate has greatly surprised me.
...
Level 5 languages must provide *native* support for holon (system) composition as a first-class language feature, rather than requiring external frameworks that provide DSL upon OOP language. This means the compiler can perform static checks on system composition, rather than relying on runtime interpretation or compiler extension.


The `ApplicationContext` can technically be instantiated multiple times. For example, in parent-child hierarchies or test suites, but it lacks true compositional isolation. Parent-child contexts form a leaky hierarchy where child contexts resolve missing beans from parents, preventing independent subsystem reasoning. Most beans are JVM-wide singletons, so multiple contexts often inadvertently share state or configuration. This is not a limitation of instantiation; it is a limitation of compositional semantics. Spring introduces Level 5 wiring but restricts it to a Level 2 reasoning topology. Developers must trace dependencies across a globally shared resolution space, causing O(N²) interaction complexity.


=== Why Isn't Level 5 Here Yet?
Several factors delay the adoption of native holon composition:

* **Primordial Soup inertia**: Existing DI frameworks work *well enough* with hacks and conventions. Rewriting them with compile-time guarantees requires breaking compatibility.
* **Perceived mundanity**: Dependency injection looks "too simple" to be a new paradigm. But so did structured programming in 1968.
* **Complexity pressure threshold**: Academic projects rarely reach the scale where Level 5 benefits become obvious. Industry projects do, but are locked into existing ecosystems, which are perceived as "normal".
* **Incremental adoption path**: A clean-slate Level 5 language requires a migration strategy. However, as we could learn from Java language experience, higher level languages, should support lower levels constructs well, so level 5 language must be a good level 4 language.

One plausible transition path resembles the move from OOP-in-C to OOP-in-C++: implicit constructs and design patterns gradually replaced by explicit language features. However, evolution is path-dependent, and alternative trajectories are possible.
...
The line `in JdbcTemplate;` is shortcut for `in val jdbcTemplate : JdbcTemplate`. However, 99% of names used in a component for DI match the name of the type. So providing explicit name for dependency is an exception. `@TransactionAware` is aspect packages external dependencies and such aspect convert kind of converts dependency to component feature.

The module that provides multiple DAO components might look like the following:
...
holon class MyEditPlugin with {
    ...
    // used as the last argument of the function
    actionManager.action(#name "EditMyFile") <> {
        // this holon-lambda implements `facet { in ActionContext; out Action; }`
        // action that uses components from the lexical scope,
        // it is lazily executed by action manager
        // in ActionContext is provided by the facet type implicitly
...
holon trait GeneratedFacetType {
    // out-function that returns a result and depends on ActionContext;
...
New level also deprecates some elements or makes them unfashionable. Like `GO TO` was deprecated in structured languages.
...
| The system dependencies are explicit, and it is easy to select needed component configuration
...
There are the following major steps in the evaluation procedure:

=== Analysis Steps
==== Selecting Idiomatic Programs

The goal of this step is identifying the bodies of the code that are idiomatic to the language. On this step it supposed to select code that:

* Follow industry guidelines and practices for the good code for this language if there are any.
* Fairly large for the language. For code base to successfully grow in size, it should implement some good practices that would allow it to grow in size.
* "Normal" programs for the language should be preferred in contrast to the programs that heavily innovate and explore some new ideas in the program design
* If project uses internal and external DSLs, the care should be taken to understand what part of the code is being analyzed.

==== Key Abstraction Identification

The goal of this step is selecting concepts that are on the growth complexity path. Such concepts form the top level of the language. The number of such abstractions increase in number with growth of the complexity, and they are not limited or contained by other abstraction.

A language only advances to a new level when the compiler or runtime absorbs the cognitive mapping cost of that connection type. Features that require developer discipline to emulate (e.g., manual virtual tables, convention-based dependency injection, explicit state passing) remain DSL-emulated: they appear in code but don't reduce mental overhead, so the growth path stalls at the previous level.
...
The goal of this step is measuring the level of the links identified in the previous step and the frequency of their appearance.

For example, In a 50k LOC C codebase, cognitive load grows with the number of global functions and struct definitions (Level 3 connections). In an equivalent Java codebase, load scales with interfaces, dependency graphs, and generic contracts (Level 4 connections).

For example, the statements are abstractions on the growth path for unstructured BASIC and FORTRAN 66, because the program usually grows by adding global statements. However, for Visual BASIC and FORTRAN 77, statements are mostly contained in procedures and functions, so when reasoning about the program we could analyze the entire procedure and save assertions about it, ignoring statement content after reaching to the conclusion.

The 'growth path' isn't derived from language specs, but from what has demonstrably scaled in practice. This keeps the framework anchored to engineering reality rather than formal semantics.

==== Overall Level

The last step is checking that reasoning that corresponds to the specific level is possible using identified links and concepts and this level of reasoning is applicable to idiomatic programs. It is also a good point to check the completeness of the level. For example, FORTRAN 66 had procedures, but it did not support recursion. Only FORTRAN 90 finally officially enabled recursion with 'RECURSIVE' keyword. And only FORTRAN 2018 made procedures recursion-capable by default.

[NOTE]
====
**Diagnostic Question**: When evaluating a language or technology, ask: *"Is this abstraction meant to scale human reasoning (keep at Level 3+), or optimize machine execution (erase to Level 2)?"* If a feature forces Level 1/2 reasoning on complex domains (manual state tracking, flat global scope, copy-pasted logic), cognitive load will grow fast. If it supports Level 3+ composition with hierarchical structure and/or explicit contracts, reasoning scales more gradually. This boundary condition explains why compilation stops at machine code, why optimization erases abstraction, and why the tension between human reasoning and machine execution is structural, not accidental.
====

=== Issues with Measuring Procedure

There are kinds of trivial containment that does not add structure over contained elements. For example, formally, Java packages contain classes and interfaces. However, the packages are just name prefixes for Java class names, so a composite name could be formed. There is still a global flat namespace for classes, and we still reason about individual classes, packages are trivial groups of classes, not providing useful operations over them. Some libraries like Spring Framework allow some package-wide operations, but this is done by interpreters of system definition language that use Java as a substrate language.

The step of identifying concepts on the growth path is semiformal and has subjective elements.

* Idiomatic programs needs to be considered, because "the determined Real Programmer can write FORTRAN programs in any language". It is also possible to write procedural code in OOP languages. The step of selecting idiomatic program is somewhat subjective. The understanding of good OOP stabilized only recently, and there are still disagreements on details or applicability of the principles. However, following industry standards and code size will reduce selection subjectivity. The programs should be good enough to grow in size, and industry standards would allow to filter out code that use language in non-intended way. While analysis of a single program could introduce wrong data, analyzing multiple programs will allow reducing subjectivity of the step.
...
After that, we need to examine how concepts on the growth path are connected to each other and what is the level of connections on this growth path. The maximum level of connections on the growth path determines the level of the language. For example in BASIC, the 'LET' statement has an expression as its part (level 3) and has a link to the next statement (level 2). However, the expression is not on the growth path, but the link to the next statement is. So while the concept is at level 3, on the growth path it behaves as level 2.

However, it might happen that there is a mix of different levels on the growth path. If lower level concepts dominate, we could mark the language as intermediate between levels, or just ignore rarely used high-level concepts. Such intermediate languages might have elements of a higher level that seem to be on growth path, but such features might be underused for one reason or other (for example, it has too many restrictions or does not fit the rest of the language well). For example, formally FORTRAN 66 had procedures, but their implementation was plagued with limitations, and they were rarely used in idiomatic code. When creating a teaching language BASIC at that time, the authors of BASIC even dropped procedures as non-essential element, instead there were `GO SUB` and `RETURN` statements that worked using line numbers and were level 2 concepts.

=== Step Summary

1. *Select idiomatic programs* – Choose large, real‑world codebases that represent typical usage.

2. *Identify key abstractions* – Find which concepts grow in number as program size increases.

3. *Analyze connections* – Determine the level (2,3,4) of links between those abstractions.

4. *Assign level* – The highest connection level on the growth path determines the language’s level.

5. *Validate reasoning* – Check that the expected reasoning style (storytelling, first‑order logic, etc.) is actually used by developers.

== Level 6 and Later

The proposal that system/holon-oriented programming is the next level of the programming languages might look stretched at the first sight. However, I tried to refute it for many years, and I still have not found anything else to replace it or supersede it as a more generic mechanism. I think that something "language-integrated dependency injection" will eventually sneak into both new and old languages (for example, context-oriented programming in Kotlin is an example of half-step in this direction). And after that, there will finally be hope to understand what will be a new level 6. There will be some abstractions that will use holons as lower-level building blocks, and there will be even the better tools to manage complexity of software.
...
I think that increasing level of abstractions is a main evolution line because it critically affects the way we are reasoning about programs. However, within a single level, a horizontal evolution happens as well. The horizontal evolution makes languages more fit to some domain, while losing some part of general-purpose features.

For an extreme example, ISO Prolog is a level 3 language that is very fit for some class of finite domain search tasks, but it is much less general-purpose than C or Pascal, and it is difficult to write an OS in it.
...
Within a single level, languages also evolve to adopt more compact notation for frequently used features. For example, many current OOP languages adopted FP-like syntax for single-method objects. While it does not add a new way of reasoning, it enables to use OOP in scenarios where it was too verbose before. So such small-scale evolution reduces cost of reasoning as well.

Abstraction level predicts cognitive scalability only when matched to the right complexity type. Haskell's Level 4 constructs (algebraic data types, monadic effects) excel at compiler-like structural complexity but offer no advantage for stateful, integration-heavy data-entry tasks. This is not a failure of Level 4. It is evidence that languages evolve under specific domain pressure, and their growth-path abstractions reflect the complexity they were built to face. A Level 5 language for enterprise systems must address system composition and lifecycle management; a Level 5 language for proof engineering must address theory dependencies and proof reuse. The level is necessary but not sufficient for domain fit.

The extreme case of horizontal development is DSLs that lose general-purpose computation capability but provide good abstractions for some specific domain. However, these languages consists of some concepts that are connected to each other. The level of these concepts and the level of connections will also define the way we reason about the language and its cognitive scalability. For example, BPMN is a level 2 language with some level 3 elements. It has quite poor cognitive scaling as a result.

For example, quote from report on BPMN indicate this:
...
BPMN (Level 2 with Level 3 islands) illustrates the 'beautiful when small, ugly when grown up' pattern: process diagrams are intuitive for <50 activities, but cognitive load explodes when modeling enterprise workflows because the growth path remains flat global references. This validates the framework's core thesis: abstraction level predicts scalability, not initial usability.
...
3. There are also plan to write articles that discuss applicability of the abstraction classification scale to the complexity challenges related to DSLs with focus on BPMN and context-free grammars.

== Conclusion

In this article, I've tried to demonstrate that the abstraction levels of programming languages could be classified by semiformal process. The resulting level is more detailed than "high" and "low". The measuring process has subjective elements, but I believe that some local consensus could be reached on these subjective elements.

This framework can provide valuable lens for understanding language evolution. By focusing on connection types rather than paradigms, we can semi-formally identify when a language truly represents a new level of abstraction. The key insight is that each new level reduces cognitive complexity growth by adding a new way to reason about programs without needing to consider all details at once.

*Practical takeaway*: Use the framework diagnostically, if maintenance costs grow non‑linearly, check whether you've exhausted your language's growth‑path abstractions or whether a higher‑ceiling language would better sustain long‑term maintainability.

Caveats on using the scale:

* A language’s abstraction level does not guarantee reduced cognitive load for every task. For state‑iterative problems (e.g., web servers, databases), a Level 4 pure functional language may require more mental translation steps than a Level 4 imperative language like Go. Task‑language fit matters as well.

* Abstraction features reduce cognitive complexity only when used correctly. For example, for level 4, violations of SOLID principles (fat interfaces, substitutability failures, etc.) can turn black‑box abstraction into a source of accidental complexity. The framework assumes expert, idiomatic usage along the growth path.
...
*Foundational boundary*: Level 2 is the lowest layer where sequential reasoning is preserved; Level 1 is direct human execution or hardware interpretation that erases order for performance (see https://en.wikipedia.org/wiki/Computer_(occupation)[Computer (occupation)]). All higher levels exist to lift humans out of Level 2 (or manual state tracking at Level 1). When languages, databases, or proof assistants force Level 1/2 reasoning on complex domains, cognitive load grows fast. When they support Level 3+ composition with explicit contracts, reasoning cost scales more gradually. This boundary condition explains why compilation stops at machine code, why optimization erases abstraction, and why the tension between human reasoning and machine execution is structural, not accidental.
...
* HOP: Holon‑Oriented Programming
...
The framework presented in this article did not emerge from a blank slate. It was directly inspired by **theories of human reasoning**, specifically:

- https://en.wikipedia.org/wiki/Piaget%27s_theory_of_cognitive_development[Jean Piaget's theory of cognitive development] (sensorimotor, preoperational, concrete operational, formal operational stages). Piaget showed that children's reasoning matures through qualitatively distinct levels, each characterized by new ways of manipulating mental representations.

- https://en.wikipedia.org/wiki/Model_of_hierarchical_complexity[Michael Commons' Model of Hierarchical Complexity] (systematic, metasystematic, paradigmatic, cross‑paradigmatic stages). Commons extended Piaget's work by formalizing the *complexity of tasks that a reasoner can handle, in the context of some domain*. His model provides explicit axioms for ordering stages by the hierarchical order of actions.

These researchers did not merely describe interesting parallels – they **defined and attempted to formalize the very notion of reasoning stages**. That was a major scientific contribution. Their work proves that human cognition itself is stratified into discrete levels of increasing complexity.

What I have done in this article is **reverse‑engineer why and how the same level structure appears in programming languages**. The stages of reasoning identified by Piaget and Commons are not arbitrary; they reflect structural properties of any system that must manage complexity through hierarchical composition and bounded abstraction. Programming languages – as human‑designed artifacts for managing computational complexity – naturally converge on the same ordinal scale because they must match the cognitive constraints of their users.

Thus, the correspondence between language levels and psychological stages is not a mere analogy or a post‑hoc mapping. It is evidence that the **scale of abstraction is grounded in human cognition**. Piaget and Commons gave us the map of reasoning complexity; this article translates that map into the technical language of connection types, growth paths, and system composition.

The table below summarizes the correspondence (the first four stages align with Levels 1‑4; later stages are extrapolated):
...
The correspondence is strong for Levels 1‑4, providing cross‑validation that the abstraction level scale is not arbitrary. For Level 5 (system/holon‑oriented programming), the mapping to Commons' systematic/metasystematic stages is plausible but not yet confirmed by mature language implementations. For Level 6, any connection to Commons' higher stages remains speculative.

The paradigmatic and cross-paradigmatic stages present greater interpretive challenges. I have major doubts about the description of these stages given by M. Commons. Examples used in his articles could be interpreted in different ways. However, some wording suggests that the language-oriented paradigm might correspond to this stage. However, the language-oriented paradigm works at the language and source level (compile time), rather than runtime, so it does not look like a continuation of the mainstream evolution line. And LOP functions as horizontal rather than vertical development in language evolution. I also do not think that the usefulness of the system-oriented paradigm has been sufficiently exhausted for the next paradigm to start to appear, but it still could be an interesting area to investigate.

The psychological models provided the initial insight that led to the connection type analysis. However, the framework does not depend on psychology for its validity; it stands on its own technical description of link topologies, growth path analysis, and cognitive scaling.

Detailed correspondence between paradigms and stages deserves more extensive work than is feasible here, so I leave it here mostly as a short side note.

[appendix]
== Cognitive Foundations: Operative Memory and Chunking

This appendix provides formal cognitive foundations for readers who want the mechanism behind the article's diagnostic lens. The main text sometimes uses "working memory" as a single bottleneck for simplicity. However, cognitive psychology distinguishes at least three interacting systems that matter for programming:

* **Short‑term memory (STM)** – a limited‑capacity store holding roughly 4–7 *discrete, unstructured items* (e.g., digits, variable names).
** Miller (1956): link:https://cse.buffalo.edu/~rapaport/575/F01/miller56.html["The Magical Number Seven, Plus or Minus Two"]
** Cowan (2001): link:https://www.semanticscholar.org/paper/The-magical-number-4-in-short-term-memory%253A-A-of-Cowan/c8f359b3967ddef8e6d7f6ad58213a543d33ea22?p2df["The magical number 4 in short‑term memory"]

* **Long‑term memory (LTM)** – a store in which knowledge is held indefinitely (language semantics, framework patterns, team conventions). Access is slower but capacity is effectively unlimited.
** Overview: link:https://en.wikipedia.org/wiki/Long-term_memory[Long‑term memory – Wikipedia]

* **Operative memory** (Galperin, Leontiev, and others from the Soviet activity‑theory school) – a *task‑bound workspace* where developers actively manipulate *structured chunks* of knowledge (e.g., an entire algorithm, a class interface, a dependency graph). Unlike flat STM, operative memory can hold complex subgraphs – *if* the code's abstractions support chunking.
** Scribner (1985): link:https://lchcautobio.ucsd.edu/wp-content/uploads/2017/04/AnActivityTheoryApproach-to-Memory.pdf["An Activity Theory Approach to Memory"]
** Zinchenko (2009): link:https://psy-journal.hse.ru/data/2017/01/12/1115348629/2_Zinchenko_LIVING_MEMORY_IN_P_I_ZINCHENKO_S_RESEARCH.pdf["Living Memory in P.I. Zinchenko's Research"]
** Supplementary: link:https://en.wikipedia.org/wiki/Chunking_(psychology)[Chunking (psychology) – Wikipedia], link:https://en.wikipedia.org/wiki/Working_memory[Working memory – Wikipedia]

**Why this distinction matters for abstraction levels**
A well‑designed contract (interface, dependency declaration, abstraction boundary) is not merely "one less item to remember". It is a *chunk boundary* that allows operative memory to treat an entire subgraph (e.g., a whole module, a JSON decoder, a transaction manager) as a single, manipulable unit. When abstractions leak – e.g., when you need to know implementation details to use an interface – operative memory fragments back into flat item‑tracking, and cognitive load spikes *non‑linearly*.

**The cost of context switching as indirect supporting evidence for operative memory**
Empirical research shows that task switching in software development imposes a significant load on operative memory, directly affecting productivity. The foundational finding (link:https://ics.uci.edu/~gmark/chi08-mark.pdf[Mark et al., CHI 2008]) established that developers need time to recover from interruptions, and that interruptions have a negative effect on the quality of work — increasing stress, frustration, and time pressure while reducing attentional continuity. This suggests that there exists a type of memory in the human mind that is distinct from short‑term memory (because it has greater capacity) and distinct from long‑term memory (because reconstructing task‑bound structures requires effort after context switches).

Higher‑level languages better support chunking and scope reduction. By providing explicit boundaries (blocks, functions, modules, interfaces), they let operative memory work with larger, structured units – reducing the need to track low‑level details and keeping cognitive load manageable as systems grow.

[appendix]
== Other Domains
This article considers concept evolution for complexity management in programming languages. However, the evolution with similar structure could be seen in other domains as well.

The reason is structural: Level 2 is the universal reasoning substrate. Databases flatten results to tabular rows (Level 2), proof assistants accumulate hypotheses into global contexts (Level 2), and compilers reduce holons to sequential instruction streams (Level 2). Level 1 execution, whether by silicon, a database engine, or a proof kernel, then erases order entirely for performance. Abstraction levels scale human reasoning; lower levels erase it for execution. The tension is structural, not accidental.
...
NOTE: This appendix examines Coq's module and typeclass systems as potential compositional mechanisms, not Coq's core logic or type theory.

The abstraction‑level framework applies beyond general‑purpose programming and data systems. In proof assistants like Coq, we could also observe that complexity management tools with high potential are restricted by their implementation.

Coq provides `Module Type` (theory/interface), `Module` (model/implementation), and Functors (parameterized contracts). Yet these operate in a syntactic layer separate from the core proof language (the Calculus of Inductive Constructions, CIC). A module cannot be passed as a proof term, stored in a variable, or injected contextually into a proof script. **Module types are not first‑class in the term language**; while they define contracts, they cannot be referenced directly as hypotheses inside a proof.

Crucially, the module system is **not on the primary growth path for most developments**, though its role depends on development style. In script‑style Coq (common for interactive proofs), the growth path consists of lemmas, definitions, and tactics operating in an accumulating global context. In library‑style Coq (e.g., CompCert, MathComp), functors and module types do structure theories, but they remain orthogonal to term‑level reasoning. Coq’s typeclass mechanism and MathComp’s canonical structures provide partial Level 4 composition via implicit dictionary passing and coercions, yet they lack explicit `in`/`out` boundaries and remain tightly coupled to the global typeclass resolution engine. Unlike Java interfaces or Rust traits, which are referenceable, composable, and form the compositional backbone, Coq’s module and typeclass systems serve as packaging and parameterization utilities, not native compositional units for proof terms.

[cols="2,2,2,2",options="header"]
|===
| Framework Dimension
| Coq's Current Capability
| Reasoning Pattern
| Level Classification

| Name Resolution
| `Require`/`Import` + `Section`/`Local`
| Lexical scoping with global accumulation fallback
| Level 2/3 hybrid

| Composition
| Functors + explicit instantiation
| Parameterized theory assembly
| Level 3 (structural, not contextual)

| Reasoning Scope
| Global context + opt‑in `Local`
| Script‑style: accumulating; Library‑style: section‑bounded
| Level 2 default

| Automation
| `HintDb` + Typeclasses + `Local Hint`
| Global search with opt‑in scoping
| Level 2/3 hybrid
|===

==== Historical context

Coq's module system emerged when logical consistency and expressive power were dominant concerns; large‑scale team development and long‑lived codebases became prominent pressures later, revealing scaling limits in the original design. Only recently (with verified compilers, large proof libraries, and industrial adoption) Coq has encountered complexity pressures comparable to enterprise software. The module system, designed for a smaller world, now reveals its limits when proof artifacts must be composed, reused, and maintained by distributed teams.

==== Practical consequences

The flat‑resolution pattern leads to concrete problems that mirror Level 2 scaling symptoms:

- Induction principles for nested inductive types often require manual boilerplate, because module boundaries cannot hide internal details or export scoped induction schemes without explicit functor instantiation.
- Temporary axioms added for convenience pollute the global context; colleagues accidentally depend on unproven assumptions because there is no dependency declaration to make boundaries explicit.
- Parallel work is hampered: two developers cannot independently refine different implementations of the same theory without risking name clashes or implicit cross‑dependencies, as there is no first‑class mechanism to inject one theory into another proof context.

These are **composition problems, not logic problems**. The Curry‑Howard correspondence extends naturally to object‑oriented patterns: interfaces as theories, implementations as models, dependency injection as assuming theories as hypotheses. Coq's module system was a pioneering step, but it remains a Level‑3 solution for organizing theories, not a Level‑4 or Level‑5 solution for composing proofs at the term level.

==== What a Level‑5 Proof Assistant Could Look Like

In a Level‑5 proof assistant, proofs become **holons**: scoped, dependency‑declared composition units with explicit contracts. The primary contract units are **theories** — named collections of types, operations, and laws (e.g., `Monoid`, `Group`). A holon `in`-s the theories it assumes, and `out`-s either a refined theory, a derived structure, or a verified implementation. Individual lemmas are `local` to a holon unless explicitly promoted to `out`.

Crucially, a dependency is not an abstract theory; it is a **theory parametrized by types and operations** (generic or concrete). For example, a proof about natural numbers may need both the additive monoid `Monoid(Nat, +)` and the multiplicative monoid `Monoid(Nat, *)` as separate, named dependencies. This mirrors dependency injection in software systems, where type arguments act as implicit qualifiers: `ObjectProvider<DataSource>` vs. `ObjectProvider<KafkaTemplate>` are distinguished by their generic parameter. Similarly, a holon’s `in Monoid(Nat, +)` and `in Monoid(Nat, *)` are distinguished by the operation, even though both satisfy `Monoid(Nat, _)`.

The axiom‑pollution problem is replaced by explicit `in` dependencies on theories. A holon declares `in Monoid(Nat, +)` and checks against the theory's contract; the proof obligation is deferred to the outer holon that provides the concrete instance. This is identical to writing `class Foo { final Bar bar; }`: the class compiles against `Bar`'s interface, and the concrete instance is injected later.

This bounded environment also tames proof automation. Tactics like `auto` would **default to bounded hint scopes**, seeing only the theories that are `in` scope (plus any `local` lemmas). Cross‑boundary search would require explicit opt‑in. The scope is *large enough* to be useful, yet *limited enough* to keep search predictable — a critical property for team‑scale proof development. This contrasts with Coq's global hint databases, which often cause unpredictable search and namespace collisions as libraries accumulate.

Dependencies and lemma‑local holons can be further packaged into **aspects** — modular, composable building blocks that collectively shape a proof’s reasoning environment. Much like how `@PreAuthorize`, `@GetMapping`, and `@RequestParam` combine in Spring to define an endpoint’s execution context, multiple proof aspects (induction principles, symmetry lemmas, domain‑specific hints, or boilerplate tactics) compose to create a structured *proof space*. Instead of repeating these patterns or relying on global hint databases, developers `in` the relevant aspects, and the proof assistant assembles them into a bounded, predictable reasoning context. This makes proof automation *composable* rather than *monolithic*: each aspect contributes a slice of the available lemmas and tactics, and the combined scope is explicit, locally scoped, and free from hidden cross‑contamination.

Most team work occurs at the *non‑root* level: a developer can take a holon with unresolved `in` dependencies and prove properties that are independent of the concrete implementation. Another team member can later provide the dependencies by instantiating the holon with a specific model that satisfies the theory’s contract. Teams commit to _contracts_ and compose their work without global coordination.

A proof for a verified compiler, security proof, or standard library is not considered complete until it culminates in one or more **root holons**. Each root holon relies exclusively on system‑provided `in` dependencies and `local` components, and may optionally expose `out` results. Within its scope, the root holon grounds and connects all subordinate components, providing the structural guarantee that verification is finished. The system `in` dependencies grant privileged capabilities (such as axiom introduction or low‑level kernel interactions) that must be strictly confined. Dangerous constructs are never globally exposed. Instead, the root holon acts as a capability gate, injecting privileges only into the downstream components that require them for their specific verified scope.

Unlike Coq's current module system, which operates primarily at the file/compilation level, these holons **are on the growth path**. As a proof grows in complexity, the number of holons increases. Developers compose them via `in`/`out` contracts, not by assembling a global context. Thus, the abstraction level of a proof assistant is determined by whether its primary reasoning units are first‑class, composable holons, not by the presence of a module system for file organization.

As formal methods increasingly face larger proofs and larger teams, the pressure for Level‑5 composition will grow. Coq's pioneering module system showed the way; the next step is to make composition a first‑class part of the proof language itself, or provide a robust elaboration layer that reduces Level‑4/5 contracts to the trusted CIC kernel without breaking reasoning locality. Any such extension must preserve Coq’s small‑core soundness guarantee by treating holons as elaboration‑time contracts that emit explicit, verifiable proof terms, not by expanding the trusted computing base.

==== LLM Considerations for Dependent Types

Research on LLMs for dependent-type systems often cites the inherent difficulty of dependent types as a bottleneck. An alternative hypothesis is that the lack of proof packaging constructs may also contribute — though this relationship remains under active investigation. The theorem‑proving community traditionally favors linear tactic scripts and explicit lemma chains, which force readers (human or AI) to reconstruct proof state step‑by‑step. This is a cultural preference, not a type‑theoretic necessity.

Under the Curry‑Howard correspondence, object‑oriented interfaces, traits, and module signatures are just *existential types* and *parametric functors*. Packaging a proof behind a verified contract (`theory ProofX { type T ...; def D := ... ; lemma A : ...; lemma B : ...; }`) is logically equivalent to hiding implementation behind an interface. When DTL ecosystems underuse these packaging constructs, they flatten the reasoning graph into a global proof timeline — maximizing cognitive load and steepening the complexity curve.

**What the research shows**:

* Autoregressive generation alone fails for dependent types because the type checker rejects approximation; every step must be globally consistent link:https://arxiv.org/abs/2306.15626[LeanDojo, 2023]. Interfaces and classes in OOP are the way to represent *related partial knowledge* — we don't need global consistency, only contract‑bounded consistency. Similarly, dependency injection could replace explicit linking with restricted, context‑sensitive search, splitting reasoning into system composition and local component‑wide reasoning.
* Successful systems treat the type checker as a reasoning partner, using retrieval‑augmented lemma search, tactic‑state embeddings, and iterative refinement link:https://arxiv.org/abs/2504.21801[DeepSeek‑Prover‑V2, 2025].
* The dominant bottleneck isn't token count; it's *proof‑state representation*. LLMs perform poorly when forced to reason over flat proof scripts, but improve dramatically when given structured context (current goal, available lemmas, type constraints).

**Practical implication**: The "complexity cliff" in dependent types is partly self‑imposed. Both humans and LLMs require explicit context partitioning to avoid working‑memory saturation. The language doesn't eliminate complexity; it forces it into a form that can be partitioned, verified, and scaled. When designing proof libraries or AI‑assisted tactics, ask: *"Can this proof be held as one chunk, or must the reader de‑chunk to understand usage?"* This is the same diagnostic question that guides interface design in Level‑4 languages.

*The chicken‑and‑egg problem for proof data*:
Current AI research for proof assistants relies almost exclusively on tactic scripts because they are the only abundant human‑written proof data. Tactics are essentially editor commands that operate on the focused proof state, not declarative proof terms. Training LLMs on tactic scripts is analogous to training a Java model on IntelliJ IDEA keystrokes rather than on source files: the model learns surface command patterns but lacks the structured reasoning context that gives them meaning. Raw proof terms are too low‑level and verbose to write by hand, and existing structured proof languages (e.g., Isabelle/Isar) still lack the modular, contract‑bounded packaging needed for large‑scale composition. Without larger composable proof units, it is hard to generate the datasets that would enable LLMs to learn genuine reasoning rather than merely mimicking a proof‑state editor. The proposed structured proof language (with explicit dependency declarations and contract boundaries, e.g., `in`/`out`‑style notation) aims to break this cycle by providing a structured, human‑writable, machine‑tractable representation — turning proof engineering into a compositional, scalable activity for both humans and AI.

**Proof irrelevance enables post‑hoc structuring**:
Because dependent type systems treat proofs as *write‑once* artifacts (proof irrelevance), we can afford to convert tactic‑generated proofs into high‑level structured, composable representations *after* verification. Tactics remain the interactive editing interface; structured proofs become the durable, machine‑tractable artifact. This elaboration step yields immediate benefits:

* **Bounded proof search**: Tactics operate within scoped dependencies (e.g., declared `in` assumptions), not global hint databases — eliminating unpredictable search from environment‑side effects.
* **Faster incremental re‑verification**: Only affected proofs need re‑checking; tactic re‑execution is avoided because the structured contract is already verified.
* **Proof caching and reuse**: Verified contracts can be memoized and shared across projects without re‑elaboration.
* **High‑quality structured training data**: Once converted, structured proof units become composable, contract‑anchored examples for future LLM assistance.

This resolves the chicken‑and‑egg problem: we don't need LLMs to generate structured proofs from scratch. We generate proofs via tactics (as today), then elaborate them into high‑level proof units as a compilation step. The structured form becomes the interface; the kernel term remains the ground truth. Predictability comes from the contract; soundness comes from the kernel.

[#dynamic-typing]
[appendix]
== Dynamically-typed Languages

Dynamically-typed languages could be efficiently used on a relatively small scale and projects with short lifetimes. They are good when project is small (PoC or prototyping stage), and they allow quickly checking some ideas, they also allow not to formalize unclear parts too early.
...
Pure dynamic typing shifts type contracts from compiler-enforced boundaries to developer memory and runtime checks. Modern ecosystems mitigate this via gradual typing (TypeScript, Python), IDE-assisted inference, and contract testing, but these add toolchain overhead and don't eliminate the non-linear cognitive scaling risk in large, long-lived codebases where context shifts frequently.

Thus, a purely dynamically-typed Level 5 language faces significant cognitive-scaling challenges. Hybrid approaches (gradual typing, optional annotations) may mitigate these risks, but the trade-offs require careful design. The static typing will be likely added later anyway if language is used to develop large code bases, so doing this intermediate step will cause spending efforts later to bolt on static typing while solving conflicts and carrying some dead weight of dynamically-typed legacy.
...
Level 4 to level 5 transition might correspond to transition to some holon/system logic that would allow formalization of operations over holons/systems. This new logic may not be reducible to higher-order logic, analogous to how higher-order logic is not reducible to first-order logic — though formal verification of this claim remains future work.

This logic needs to add a new way of existential and universal quantification that will be difficult to express in the higher order logic. I currently see as a candidate a quantification over the structure of the holon/system. A kind of formalizing metaprogramming. Aspect could be formalized as meta-statements over components.

For example, the statement like the following could be directly expressed:

* The holons is structurally equivalent, if there is a bijection over indexes that maintain dependency renaming.
...
The semantic rules and associated inductive principles could be potentially formally analyzed for the level they are representing. Then conceptual heatmaps could be built by tools for idiomatic programs that check if these concepts are actually used to manage complexity. Such heatmaps could inform language evolution and program analysis by highlighting high-level concepts that are underutilized or difficult to apply — though developing reliable detection methods remains an open research question.
