...
This boundary explains why optimization often means erasing higher‑level reasoning: we trade human‑comprehensible structure for machine‑executable efficiency. The framework’s diagnostic power lies in identifying *when* erasure crosses into structural debt: when the erased boundary was necessary for local reasoning, testability, or compositional safety.
====
...
[NOTE]
====
**Optimization as Level Descent**: Performance tuning is fundamentally a process of erasing higher-level reasoning boundaries to align with lower-level execution semantics. This is not inherently negative: erasure is how we pay for speed. The framework's diagnostic power lies in identifying *when* erasure crosses into structural debt: when the erased boundary was necessary for local reasoning, testability, or compositional safety. The goal is not to preserve all levels at runtime, but to erase them at the right compilation stage, with the right policy.
====
...
However, it might happen that there is a mix of different levels on the growth path. If lower level concepts dominate, we could mark the language as intermediate between levels, or just ignore rarely used high-level concepts. Such intermediate languages might have elements of a higher level that seems to be on growth path, but such features might be underused for one reason or other (for example, it has too many restrictions or does not fit the rest of the language well). For example, formally FORTRAN 66 had procedures, but their implementation were plagued with limitations, and they were rarely used in idiomatic code. When creating a teaching language BASIC at that time, the authors of BASIC even dropped procedures as non-essential element, instead there were `GO SUB` and `RETURN` statements that worked using line numbers and were level 2 concepts.
