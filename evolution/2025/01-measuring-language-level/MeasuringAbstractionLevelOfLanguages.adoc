...
Well-designed systems maximize the *Ideal Code* quadrant. This explains the **code review paradox**: 10 lines changed trigger intense scrutiny, while 30 files touched across multiple modules often get "LGTM". This is not reviewer negligence; it is context exhaustion. Human working memory cannot hold arbitrary amounts of code; beyond a threshold, reasoning collapses into pattern-matching.
...
* Lower-level languages have lower entry costs. Their primitives are easier to grasp initially, but there are simply too many of them to coordinate at scale.
...
- **Level 2** is the sequential machine contract (ISA) that *preserves order* across steps. It abstracts away the fact that each step is stateless; it gives the illusion of a linear, ordered execution. Processors implement this contract, but internally they may execute speculatively and out‑of‑order – as long as the observable sequential semantics are preserved.
...
**Framework interpretation**: Isolating responsibilities partitions the reasoning graph, converting multiplicative interaction cost into additive cost.
...
[NOTE]
====
**Functional Programming as a bridge**: Pure FP can be viewed as a transitional stage between structured programming and OOP/FP hybrids. It excels at formalizing Level 3 constructs (algebraic data types, pattern matching) and introduces Level 4 reasoning via higher-order functions. However, its treatment of existential types (interfaces) is often less ergonomic than OOP's, which is why mainstream languages tend toward hybrid models. This is not a failure of FP, it is evidence that language evolution is driven by *practical reasoning ergonomics*, not theoretical purity.
====
...
The abstraction‑level framework applies beyond general‑purpose programming and data systems. In proof assistants like Coq, we observe the same pattern: **constructs that approach Level 4 are composed via Level 2 mechanisms**, creating asymmetric reasoning boundaries that scale poorly as proof artefacts grow.
