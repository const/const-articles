...
In a Level‑5 proof assistant, proofs become **holons** (scoped, dependency-declared composition units): composable artefacts with explicit dependencies and exports. The primary contract units are **theories**: named collections of types, operations, and laws (e.g., `Monoid`, `Group`). A holon `in`-s the theories it assumes, and `out`-s either a refined theory, a derived structure, or a verified implementation. Individual lemmas are `local` to a holon unless explicitly promoted to `out`.

Crucially, a dependency is not an abstract theory, it is a **theory parametrized by types and operations** (generic or concrete). For example, a proof about natural numbers may need both the additive monoid `Monoid(Nat, +)` and the multiplicative monoid `Monoid(Nat, *)` as separate, named dependencies. This mirrors dependency injection in software systems, where a component might require two different dependencies with different generic arguments. For example, in Spring, type argument of `ObjectProvider<T>` acts as an implicit qualifier: `ObjectProvider<DataSource>` vs. `ObjectProvider<KafkaTemplate>` are distinguished by their type argument. Similarly, a holon’s `in Monoid(Nat, +)` and `in Monoid(Nat, *)` are distinguished by the operation, even though both satisfy `Monoid(Nat, _)`.
