...
| 0.1.2   | 2026-07-15 | Incomplete fixes.
...
interface IoAction[T, R] with {
    ...
    fn doIt(resource : T) : R with { throws Exception; };
}
----
...
interface IoAction[T, R] with {
    ...
    fn doIt(resource : T) : R with { @[ThrowsException]; };
}
----
...
public final class HelloService {
    ...
    public static Disposable<HelloService> newHolon() {
        return Dispoable.of(new HelloService(), HelloService::dispose);
    }
    public String greeting(String user) {
        return "Hello, %s!".formatted(user);
    }
    private static void dispose(HelloService service) {
        System.out.println("Closing");
    }
}
----

From Java, it could be created directly using the `newHolon` method:
...
The holon could also be used with a `local` declaration, which is possible as type fields and as local variables in methods. The declaration provides a number of shortcuts, allowing omitting the factory method name in case of the `newHolon` factory method:

[source]
----
local var helloService : HelloService = [HelloService].newHolon();
local var helloService : HelloService = [HelloService]();
local var helloService = [HelloService]();
local [HelloService](); // does not create name differently
----
...
* Other holons - that can be injected implicitly, specified with `in` keyword.
* Classes, primitive types, records, and other values that are always injected explicitly. Specified with parameter keyword `?`
...
[source]
----
in val templateEngine : TemplateEngine;
in [TemplateEngine]; // no name declared
----
...
[source]
----
local val templateEngine : TemplateEngine = [TemplateEngine].newHolon();
local val greetingsService : GreetingsService = [GreetingsService].newHolon(
    #template "Hello, %s!",
    #templateEngine templateEngine
);
local val checkInService : CheckInService = [CheckInService].newHolon(#greetingsService greetingsService);
...
[source]
----
local [CheckInService](#greetingsService hiService); // using parameter name
local [CheckInService](<| hiService); // not caring about parameter name
----
...
holon class CheckInServiceFactory {
    ? greetingType : GreetingType = [GreetingType].FORMAL;
    in TemplateEngine;
    local [GreetingsService](
        #template greetingType == [GreetingType].INFORMAL ? "Hi, %s!" : "Hello, %s!");
    out CheckInService();
}
...
It is also possible to declare an alias using a `val` statement:

[source]
----
local [TemplateEngine]();
local [CheckInServiceFactory](#greetingType GreetingType.INFORMAL);
local val checkInService = ?![CheckInServiceFactory].checkInService;
...
holon class EnableTemplateEngineIfMissing {
    ...
    out fn default templateEngine() {
        out templateEngineOptional.orElseHolon<>{
            out [TemplateEngine]();
        };
    };
};
...
The compile-time and static holons are assumed to live forever after they are initialized. However, instance holons often need cleanup, for example, closing database connections or doing other actions. For that purpose, there are scopes and disposable holons.

The disposable holons do not have the modifier, but non-disposable have modifier `flyweight`. The disposable holons do not have public constructor, but have newHolon static method that returns `Disposable[H]` wrapper:

[source]
----
class final Disposable[H] with {
    @HolonType[H];
} as {
    /// The owned object that will need some disposing later
    val owned : H { @Accessor};
    /// The disposer for the object. Note, if there is checked exception thrown during disposal process,
    /// it should be wrapped into {@link HolonDestructionException}.
    val disposer : Consumer[H];
    // rest of api skipped
};
----
...
The dispose method for holons allows only subclasses of `RuntimeException` or `Error` to be thrown. Other exceptions need to be wrapped. Multiple exceptions are added to the first exception.

For wrapping non-holon disposed values, a special scoped holon `Resource` is used, which uses the close method for `AutoCloseable`, or allows providing a custom close operation. Note, if `Resource` is used with `null`, there still could be a cleanup method, and this allows creating custom cleanup actions.

[source]
----
holon Resource[T] with {
    extends DelegatingHolon[T];
} as {
    val value : T;
    val close : AutoCloseable;
    @PreDestroy { close.close(); };

    out fn public static of[T](resource : T, closeAction : AutoCloseable) : Resource[T] {
        out [Resource[T]](resource, closeAction);
    }

    out fn public static of[T](resource : T) : Resource[T] with {
        type T with { extends AutoCloseable };
    } as {
        out of(resource, resource);
    };

    fn override get() : T { return value; };
};
----
...
holon class CheckInServiceFactory {
    in TemplateEngine;
    out fn checkInService with {
        ? greetingType : GreetingType = [GreetingType].FORMAL;
    } as {
        local [GreetingsService](#template greetingType == [GreetingType].INFORMAL ? "Hi, %s!" : "Hello, %s!");
        out [CheckInService]();
    }
}
holon class LoginProcess {
...
It is possible to use the construct `import (expression).*;` modifier for factories as well, and it will expose methods to the local context. The variables for out components could also be named by type name (rather than explicitly).

[source]
----
local [CheckInServiceFactory]();
import (?![CheckInServiceFactory]).*;
