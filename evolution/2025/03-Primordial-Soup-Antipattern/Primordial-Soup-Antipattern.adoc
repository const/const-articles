...
[NOTE]
====
**Key reframing**: The problem isn't "global resolution" — it's *global-only resolution* without Level 3 structured scoping or Level 4 system-level contracts layered on top. A root context is necessary to ground composition; the antipattern emerges when that's the *only* resolution plane.
====

Spring's `ApplicationContext` **is** a Level 5 composition operator:
- Dynamically resolves dependencies from environment
- Manages lifecycles, scopes, instantiation
- Assembles behavior from declarative metadata (`@Configuration`, annotations)

**However**, it operates exclusively on *Java components*, not *systems*.
When applied to architectural composition without Level 3 structure or Level 4
contracts, it degenerates to Level 2 flat resolution.
...
Below, pseudocode for simple order management service.
...
holon class OrderDaoLayer {
    ...
    local OrderStorageUtil(); // some internal component that allows
