...
| 0.2.2   | 2026-07-15 | Minor fixes, add metaphors.
...
The first change is structural. The Spring Framework is comparable to a Viking longhouse, where all processes happen within a single massive context, isolated only by cloth and leather curtains. In contrast, a holonic structure resembles a modern office building with a strict hierarchy of floors, companies, divisions, and rooms, where business processes occur strictly in their designated places.

The second change affects the development process itself. Working with the Spring Framework is strikingly similar to alchemy. Developers pour components from various jars into a single common cauldron - the `ApplicationContext`. This mixture is then fine-tuned with annotations, configurations, and implemented interfaces. If a good recipe is followed, everything works. If not, the cauldron explodes. Fortunately, this usually happens in testing rather than in production. Experienced alchemists can deduce what went wrong simply by reading the "smoke" of vague logs and exception stack traces.

In contrast, a holonic system composition language offers an engineering approach to system architecture. Systems are constructed from distinct parts with explicit contracts, allowing each component to be independently type-checked.
